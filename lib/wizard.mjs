...
export async function runWizard(root, { flags = {}, existing = null } = {}) {
  ...
  if (!stdin.isTTY || flags.yes) {
    ...
    const anyProfileFlag = ["kind", "payments", "email", "db", "cloud", "domain", "url", "ai", "audience", "private"].some((k) => flags[k] !== undefined);
    const kind = f("kind");
    ...
    return {
      ...
      private: flags.private !== undefined ? bool(flags.private) : (isStatic ? false : (base.private ?? null)),
  ...
  if (profile.kind === "static") {
    ...
    profile.private  = await askBool("Is it private (login or invite only, should NOT show up in Google)?", base.private ? "y" : "n");
  }

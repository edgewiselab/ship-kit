...
const EXTLESS_KEEP = new Set([
  ...
  "yarn.lock", "bun.lock", "bun.lockb", ".node-version", ".tool-versions", ".renovaterc", ".replit", "replit.nix",
]);
...
export function buildContext(root, profile = {}) {
  ...
  (function walk(dir, depth) {
    ...
    for (const e of entries) {
      ...
      if (e.isDirectory()) {
        if (SKIP_DIRS.has(e.name)) continue; // note: .github is kept, CI workflows are evidence
        walk(full, depth + 1);
      } else if (e.isFile()) {
  ...
  let pkg = {};
  const deps = new Set();
  const scripts = []; // every npm script command in the tree, e.g. "node --test tests/*.test.ts"
  for (const pf of pkgFiles) {
    ...
    for (const [name, cmd] of Object.entries(j.scripts || {})) scripts.push({ file: pf, name, cmd: String(cmd) });
  }
  ...
  // In a Next.js App Router project, files under app/ are server components unless they
  // open with a "use client" directive, so the directive decides, not the folder.
  const isNext = deps.has("next");
  const USE_CLIENT = /^(?:\s|\/\/[^\n]*\n|\/\*[\s\S]*?\*\/)*['"]use client['"]/;
  const isClientFile = (f) => {
    if (!/\.(jsx?|tsx?|mjs|svelte|vue|astro)$/i.test(f) || /\.(test|spec)\./i.test(f)) return false;
    if (/(^|\/)(api|server|serverless|functions|lib\/server|actions)\//i.test(f)) return false;
    if (isNext && /(^|\/)(src\/)?app\//.test(f)) return USE_CLIENT.test(read(f) || "");
    return /(^|\/)(src|app|components|pages|features)\//i.test(f);
  };
  ...
  return {
    root, files, read, has, find, grep, any, pkg, deps, scripts, gitTracked, gitEnvInHistory, isGit, isIgnored, isClientFile,
    profile,
    dep: (...names) => names.some((n) => deps.has(n) || [...deps].some((d) => d === n || d.startsWith(n + "/"))),
    installedVersion: (name) => installedVersion(read, pkgFiles, name),
  };
}

// The version of a package the project actually runs: the lockfile first (exact), then the
// package.json range (a lower bound only). Returns { version, exact, source } or null.
export function installedVersion(read, pkgFiles, name) {
  const esc = name.replace(/[.*+?^${}()|[\]\\/]/g, "\\$&");
  const lock = read("package-lock.json");
  if (lock) {
    try {
      const j = JSON.parse(lock);
      const v = (j.packages && j.packages["node_modules/" + name] && j.packages["node_modules/" + name].version)
        || (j.dependencies && j.dependencies[name] && j.dependencies[name].version);
      if (v) return { version: v, exact: true, source: "package-lock.json" };
    } catch {}
  }
  const pnpm = read("pnpm-lock.yaml");
  if (pnpm) {
    const m = pnpm.match(new RegExp(`^\\s+['"]?/?${esc}@(\\d+\\.\\d+\\.\\d+[^'":(\\s]*)`, "m"));
    if (m) return { version: m[1], exact: true, source: "pnpm-lock.yaml" };
  }
  const yarn = read("yarn.lock");
  if (yarn) {
    const m = yarn.match(new RegExp(`^"?${esc}@[^\\n]*\\n\\s+version:? "?(\\d+\\.\\d+\\.\\d+[^"\\s]*)`, "m"));
    if (m) return { version: m[1], exact: true, source: "yarn.lock" };
  }
  const bun = read("bun.lock");
  if (bun) {
    const m = bun.match(new RegExp(`"${esc}":\\s*\\[\\s*"${esc}@(\\d+\\.\\d+\\.\\d+[^"]*)"`));
    if (m) return { version: m[1], exact: true, source: "bun.lock" };
  }
  for (const pf of pkgFiles) {
    let j;
    try { j = JSON.parse(read(pf) || "{}"); } catch { continue; }
    const spec = (j.dependencies && j.dependencies[name]) || (j.devDependencies && j.devDependencies[name]);
    if (!spec) continue;
    const m = String(spec).match(/(\d+\.\d+\.\d+)/);
    if (m) return { version: m[1], exact: /^\d/.test(String(spec).trim()), source: pf };
  }
  return null;
}

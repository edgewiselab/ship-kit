...
// Email provider APIs called directly with fetch, no SDK installed.
export const EMAIL_API_HOST = /api\.resend\.com|api\.postmarkapp\.com|api\.mailgun\.net|api\.eu\.mailgun\.net|api\.sendgrid\.com|email\.[a-z0-9-]+\.amazonaws\.com|api\.brevo\.com|api\.sendinblue\.com|api\.sparkpost\.com|app\.loops\.so\/api|send\.api\.mailtrap\.io|api\.mailersend\.com|api\.postmarkapp/i;
...
export function detectCapabilities(ctx, profile = {}) {
  ...
  const caps = {
    ...
    hasAuth: c.dep("next-auth", "@auth/core", "@clerk/nextjs", "@clerk/clerk-sdk-node", "better-auth", "lucia", "@supabase/auth-helpers-nextjs", "@supabase/ssr", "passport", "jsonwebtoken", "express-session", "iron-session", "@auth0/nextjs-auth0", "@workos-inc/node")
      || c.dep("jose", "bcrypt", "bcryptjs", "argon2", "@node-rs/argon2", "@node-rs/bcrypt", "@oslojs/jwt", "arctic", "@simplewebauthn/server")
      || c.any(/getServerSession|currentUser\(|requireAuth|getCurrentUser\(|useUser\(|auth0|clerkClient|jwtVerify\(|(read|get|verify|create)Session\(/)
      || c.any(/session[^\n]*httpOnly|httpOnly[^\n]*session/i, { exclude: [".md"] }),
    ...
    hasEmail: c.dep("resend", "@sendgrid/mail", "nodemailer", "postmark", "@loops/loops", "mailgun.js", "@aws-sdk/client-ses", "@aws-sdk/client-sesv2", "@getbrevo/brevo", "mailersend")
      || c.any(EMAIL_API_HOST, { exclude: [".md"] }),
    hasInfra: c.find(/\.tf(\.json)?$/).length > 0 || c.find(/(^|\/)Dockerfile$/i).length > 0 || c.find(/serverless\.(yml|yaml)$/).length > 0 || c.find(/(cdk\.json|pulumi\.ya?ml)$/).length > 0,
...
export function naMapFromCapabilities(caps) {
  ...
  add(!caps.hasAI, ["S18", "L7"], "No AI features found in the code");

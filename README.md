...
> Set up Ship Kit. If the folder ~/ship-kit does not exist, run: git clone https://github.com/edgewiselab/ship-kit ~/ship-kit. If it already exists, run: git -C ~/ship-kit fetch origin, then git -C ~/ship-kit reset --hard origin/main, so you are on the latest. Do not look for it anywhere else on my computer. Then scan this project for launch readiness. Open the dashboard, walk me through the launch-blockers in plain English, then stop and wait: I will decide what to fix.
...
Run the tests with `npm test` (Node's built-in runner, no install needed). What changed between versions is in [CHANGELOG.md](CHANGELOG.md).

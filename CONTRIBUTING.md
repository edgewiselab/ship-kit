...
The context object `c` gives you `c.read(path)`, `c.has(path)`, `c.find(regex)`, `c.grep(regex, opts)`, `c.any(regex, opts)`, `c.dep(...names)`, `c.installedVersion(name)` (from the lockfile), `c.pkg`, `c.gitTracked`, `c.profile`, `c.caps` (what the scan found the app does), and (when a URL was given) `c.probes`.
...
Add a test for it in `test/detectors.test.mjs`: one case that should pass, one that should fail, and one that looks similar but must not trip it (the false positive you are guarding against). The helpers build a small real project on disk:

```js
test("my check fails when X", async () => is(await detect("S14", { "package.json": pkg({ helmet: "8" }) }), "pass"));
```

Then run the tests, and scan a real project before opening a PR:

...
**The standard checks.** A set of pre-launch checks circulates widely for indie and AI-built apps: turn on row-level security, enforce the paywall on the server, do not expose `.env`, set up SPF/DKIM/DMARC, add an og image, run PageSpeed, install analytics, publish a Terms of Service. These are common practice, not any one person's invention. Ship Kit compiles them (about a third of the 115 checks), keeps the useful shape of each (what to check, how, what bad looks like, why it matters), and corrects a handful with sharper, real-world guidance.

**Production experience.** A pre-launch list only covers the hour before you go live. The most expensive bugs live just outside that window: in how you build, how you deploy, and how you operate afterwards. So Edgewise Lab expanded the list with lessons drawn from a portfolio of real production apps built with AI coding agents (payments, voice-AI, marketing sites, internal tools), organised into 78 additional checks and five new sections:

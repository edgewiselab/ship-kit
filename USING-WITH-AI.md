...
1. Get the latest Ship Kit: if ~/ship-kit does not exist, run `git clone https://github.com/edgewiselab/ship-kit ~/ship-kit`. If it already exists, run `git -C ~/ship-kit fetch origin` and then `git -C ~/ship-kit reset --hard origin/main` so you are on the newest version (the checks improve often).

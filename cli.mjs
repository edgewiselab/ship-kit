...
function help() {
  log(`\n  ${C.b("Ship Kit")} ${C.dim("v" + VERSION + " · launch-readiness scanner")}\n`);
  ...
  log(`    --version         print the Ship Kit version`);
  ...
  log(`    --private true|false   ${C.dim("(login-only app: skips sitemap/indexing checks; auto-detected from --url)")}`);
...
async function scan(args, { configOnly = false } = {}) {
  ...
  log(`\n  ${C.b("Ship Kit")} ${C.dim("v" + VERSION)} scanning ${C.c(root)}`);
...
if (cmd === "version" || args.flags.version) log(VERSION);
else if (cmd === "help" || args.flags.help) help();
